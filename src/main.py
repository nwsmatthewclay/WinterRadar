...
def main() -> None:
    ...
    try:
        ...
        # A RAP profile can decode successfully but still produce zero usable
        # phase cells if the sampled vertical fields are empty/misaligned.
        # Treat that condition the same way as a phase-engine failure rather
        # than publishing an all-transparent phase/composite raster.
        #
        # The conservative MRMS classifier is already part of the operational
        # pipeline and is the appropriate fallback for this specific failure.
        rap_phase_cells = int(phase_diagnostics.get("phase_cells", 0))
        rap_diag_cells = int(
            phase_diagnostics.get("diagnostic_grid", {}).get("valid_cells", 0)
        )
        precip_pixels = int(phase_diagnostics.get("precip_pixels", 0))
        if precip_pixels > 0 and (rap_phase_cells == 0 or rap_diag_cells == 0):
            reason = (
                f"RAP phase returned no usable cells "
                f"(phase_cells={rap_phase_cells}, diagnostic_valid_cells={rap_diag_cells}, "
                f"precip_pixels={precip_pixels})"
            )
            print(f"  WARNING: {reason}; using conservative MRMS phase fallback.")
            phase_error = reason
            ...
            phase_status = "fallback_initial_no_valid_rap_phase"
