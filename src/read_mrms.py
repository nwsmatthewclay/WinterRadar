...
def _read_one(path: Path):
    """Read one MRMS GRIB message and release the native handle immediately."""
    grbs = None
    try:
        grbs = pygrib.open(str(path))
        grb = grbs.message(1)
        data = np.asarray(grb.values, dtype=np.float32).copy()
        lats, lons = grb.latlons()
        lats = np.asarray(lats, dtype=np.float64).copy()
        lons = np.asarray(lons, dtype=np.float64).copy()

        valid_time = None
        for key in ("validityDate", "validityTime"):
            if key not in grb:
                break
        try:
            ...
            try:
                ...
                try:
                    ...
                    forecast = 0
                valid_time = (dt + timedelta(hours=forecast)).isoformat()
            except Exception:
                pass
    finally:
        if grbs is not None:
...
def get_variable_name(ds) -> str:
    """Compatibility helper for callers that previously passed an xarray Dataset."""
    candidates = []
    for name in getattr(ds, "data_vars", {}):
        if name not in {"latitude", "longitude"}:
            candidates.append(name)
    if not candidates:
        raise RuntimeError("No 2-D meteorological data variable found.")
    return candidates[0]
...
def get_valid_time(path: Path) -> str | None:
    """Extract MRMS valid time through pygrib without importing cfgrib/eccodes."""
...
def get_metadata(path: Path) -> dict:
    ...
    try:
        ...
        result = {
            "product": product,
            "variable": str(grb.get("shortName", "")),
            "expected_units": field_info(product)["units"],
        }
        interesting = (
            "shortName", "name", "units", "cfName", "cfVarName",
            "paramId", "typeOfLevel", "gridType", "Nx", "Ny",
            "Ni", "Nj", "dataDate", "dataTime", "validityDate",
            "validityTime",
        )
...
def read_grib(path: Path):
    ...
    Core processing no longer uses an xarray/cfgrib Dataset. This function
    intentionally returns a small detached dictionary for any legacy caller
    that only needs the decoded 2-D field and coordinates.
    """
    data, lats, lons, valid_time = _read_one(path)
    return {
        "data": data,
